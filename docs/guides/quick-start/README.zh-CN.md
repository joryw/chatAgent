...
# 默认对话模式（chat 或 agent）
# - chat: 标准对话模式，可手动控制联网搜索
# - agent: 自主决策模式，带 ReAct 循环的 Agent
DEFAULT_MODE=chat
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `/help` | 显示可用命令和使用指南 | `/help` |
| `/config` | 查看当前模型和模式配置 | `/config` |
| `/mode <chat\|agent>` | 切换对话模式 | `/mode agent` |
| `/switch <provider>` | 切换模型提供商 | `/switch deepseek` |
| `/search <on\|off>` | 启用/禁用联网搜索（仅 Chat 模式） | `/search on` |
| `/reset` | 清除对话历史 | `/reset` |
...
自主决策的 AI Agent，能够智能使用工具。

**特点**:
- 🧠 **自主决策**: 模型自动判断是否需要搜索
- 🔄 **ReAct 循环**: 思考 → 行动 → 观察 → 重复
- 🛠️ **工具集成**: 自动使用搜索和其他工具
- 👁️ **过程可视化**: 实时显示 Agent 的思考、行动和观察过程
- 🎯 **多步推理**: 支持迭代搜索和精炼
...
当你在 Agent 模式下提问时：
1. **💭 思考**: Agent 分析问题并制定计划
2. **🛠️ 行动**: 如需要，调用搜索工具获取信息
3. **💡 观察**: 处理搜索结果
4. **🔁 迭代**: 如需更多信息则重复
5. **✅ 回答**: 提供带有引用的最终答案
...
- 需要实时信息的问题
- 需要多个来源的复杂查询
- 不确定是否需要搜索时（让 Agent 决定）
- 研究和事实核查任务

**示例**:
```
用户: "对比一下 OpenAI、Anthropic 和 DeepSeek 的最新 AI 模型"

Agent 过程:
├─ 💭 思考: 需要获取最新模型信息
├─ 🛠️ 搜索: "latest AI models 2024 OpenAI Anthropic DeepSeek"
├─ 💡 观察: 找到 5 个结果，包含模型对比
├─ 💭 思考: 信息充足，可以回答
└─ ✅ 回答: 详细对比，附带引用 [1][2][3]
```

更多详情请参阅 [Agent 模式使用指南](../agent-mode.md)。
